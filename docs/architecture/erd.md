...
**`User.teamId` допускает `null`** — у администратора компании команды может не быть. У пользователя с ролью `AGENT` или `MANAGER` команда обязательна, это проверяется на уровне сценария, а не схемы: в Postgres такое ограничение выражается CHECK-условием, но оно завязано на `roleId`, и при добавлении ролей превратится в мину.
