...
| `linked_to_existing` | `EXACT` с **другого** источника; самоимпорт | Открыть объект. Новый `SourceListing` привязан |
| `duplicate_warning` | `STRONG` или `POSSIBLE` | Открыть существующий / Отменить / Добавить всё равно |
...
**Система не агрессивна.** Агент может настоять на своём везде, кроме `EXACT`, где вторая копия бессмысленна.
...
|---|---|---|---|---|
| — | — | — | — | Набор появится в фазе 4. До реальных данных цифры здесь были бы выдумкой |

Таблица заполняется при каждом изменении весов. Пустая строка честнее придуманной.
