...
/**
 * КАЖДОЕ ДЕЙСТВИЕ ЖУРНАЛА ИМЕЕТ НАЗВАНИЕ НА ТРЁХ ЯЗЫКАХ.
 *
 * История объекта показывает не код, а фразу. Пока перевода не было, агент
 * видел `PROPERTY_STATUS_CHANGED` — латиницу капсом посреди грузинского
 * интерфейса, то есть нарушение правила 18 в самом заметном месте карточки.
 *
 * Проверка структурная и межпакетная: набор действий живёт в ядре, словари —
 * в пакете переводов, и вывести одно из другого типом нельзя. Без этого теста
 * добавленное действие молча появилось бы у агента дырой `⟦ключ⟧`.
 */
describe('журнал действий переведён', () => {
  it('у каждого кода есть фраза во всех трёх языках', async () => {
    // Импорт по пути, а не по имени пакета: корневые тесты не входят
    // в рабочее пространство и имён `@kleekto/*` не разрешают. Берётся
    // ровно два модуля — файл констант и словари, — чтобы не тянуть
    // за собой клиента базы.
    const { ACTIVITY } = await import('../packages/core/src/activity/actions');
    const { LOCALES, translate } = await import('../packages/i18n/src/index');
    ...
    for (const code of codes) {
      for (const locale of LOCALES) {
        const label = translate(locale, `activityAction.${code}` as never);

        // `translate` отдаёт `⟦ключ⟧` на неизвестный ключ — именно эту дыру
        // тест и ловит, пока она не доехала до экрана.
        expect(label, `${locale}/${code}`).not.toContain('⟦');
