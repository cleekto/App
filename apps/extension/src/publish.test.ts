...
/**
 * ЗАПОЛНЕНИЕ ДОЛЖНО БЫТЬ ПОДКЛЮЧЕНО К СТРАНИЦЕ, а не просто написано.
 *
 * Проверки выше зовут `runFill` напрямую и были зелёными всё время, пока
 * `content.ts` — единственный код, который реально работает на странице, —
 * не импортировал `publish-manager` вовсе. Заполнение существовало
 * как модуль, проходило тесты и никогда не запускалось; агент видел пустую
 * страницу там, где инструкция обещала кнопку.
 *
 * Эта проверка читает исходник и ловит ровно такой промах: сценарий,
 * который некому вызвать.
 */
describe('помощник на форме доходит до страницы', () => {
  const content = readFileSync(join(import.meta.dirname, 'content', 'content.ts'), 'utf8');

  it('content.ts вызывает заполнение, а не только объявляет его', () => {
    expect(content).toMatch(/from '\.\.\/core\/publish-manager'/u);
    expect(content).toMatch(/runFill\(/u);
  });

  it('форма распознаётся отдельно от страницы объявления', () => {
    // Страница «новое объявление» не является объявлением: проверка
    // `adapterFor` её отбрасывает, и без отдельной ветки помощник
    // не запускался бы никогда.
    expect(content).toMatch(/publishAdapterFor\(location\.href\)/u);
  });

  it('объект берётся из метки в адресе, а не угадывается', () => {
    expect(content).toMatch(/propertyMark\(location\.href\)/u);
  });
