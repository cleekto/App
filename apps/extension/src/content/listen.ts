 ...
 *
 * По одному признаку на площадку, и оба взяты из самих данных: `user_type`
 * есть в каждой карточке myhome, `realStateItemModel` — имя списка у ss.ge.
 * Требовать что-то ещё было бы ошибкой: ключ кэша `statements` есть только
 * в ответе перехода Next.js, а в ответе собственного API площадки его нет,
 * и половина сбора отсекалась бы молча.
 *
 * Сюда проходит КАНДИДАТ, а не находка: решает разбор.
 */
const MARKERS = ['user_type', 'realStateItemModel'];

function interesting(body: string): boolean {
  return body.length <= MAX_BYTES && MARKERS.some((marker) => body.includes(marker));
}
