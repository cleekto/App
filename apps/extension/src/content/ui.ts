...
export class Ui {
  ...
  /**
   * Ждём, пока агент дойдёт до полей.
   *
   * Размещение на ss.ge — мастер: сначала карточками выбирают категорию,
   * тип и сделку, и только потом появляются поля. Молчать здесь нельзя —
   * агент нажал «Разместить» и вправе знать, что помощник на месте
   * и ждёт своей очереди, а не отвалился.
   */
  waitingForForm(): void {
    const panel = this.clear();
    panel.append(this.closeButton(), this.head(this.t('extension.fill.waiting'), 'quiet'));
  }
