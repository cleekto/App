...
export interface ImportOptions {
  ...
  /**
   * Номер, вписанный агентом руками.
   *
   * ЗАЧЕМ ОН ЕСТЬ. Правило 11 запрещает расширению добывать номер самому —
   * и это остаётся в силе. Но у правила был побочный эффект, которого никто
   * не хотел: если со страницы номер не вычитывается, агент упирался в тупик.
   * А он к этому моменту УЖЕ ПОЗВОНИЛ собственнику и получил согласие —
   * номер у него в журнале вызовов.
   *
   * Тупик возникал не когда номера нет, а когда мы не смогли его прочитать:
   * площадка сменила вёрстку, показала номер картинкой, отдала его через
   * форму обратной связи. Это наша слепота, и расплачиваться за неё
   * несделанной работой агенту не за что.
   *
   * Правило 11 цело: номер по-прежнему приходит от человека, а не от нас.
   * Мы не имитируем нажатие «показать», не дёргаем чужие endpoint'ы
   * и не угадываем. Мы только принимаем то, что человек напечатал.
   *
   * Проверка та же самая, что и для номера со страницы: маска и непохожий
   * на грузинский мобильный набор цифр не проходят. Иначе руками можно было
   * бы вписать «звоните в агентство» — и сломать дедупликацию всей команде.
   */
...
export async function runImport(
   ...
   *
   * Со страницы номер берётся ТОЛЬКО если агент раскрыл его сам. Ни при
   * каких условиях расширение не имитирует нажатие «показать номер»
   * и не спрашивает его у площадки в обход человека.
   */
  const fromPage = adapter.isPhoneRevealed(document)
    ? usablePhone(extraction.payload.owner.phones)
    : null;

  /*
   * Не прочиталось — принимаем набранное руками. Проверка та же самая:
   * `usablePhone` не различает, откуда пришла строка, и маску отсекает
   * одинаково. Отдельного, более мягкого пути для ручного ввода нет
   * намеренно — он бы и стал дырой.
   */
  const typed = options.manualPhone ?? '';
  const phone = fromPage ?? usablePhone([typed]);
