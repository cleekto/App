...
/**
 * Название стадии воронки на языке человека.
 *
 * БЕРЁТСЯ ИЗ ДАННЫХ, А НЕ ИЗ СЛОВАРЯ. Раньше имя переводилось по коду,
 * и это работало ровно до первого переименования: переименованная стадия
 * показывалась одним словом всем — на языке того, кто её правил. Грузинский
 * агент видел русское слово посреди грузинского интерфейса (решение
 * владельца 2026-09-03).
 *
 * Теперь у стадии своё имя на каждый язык, а `name` — запасное: то, что
 * напечатали, заводя стадию. Оно показывается там, где перевода ещё нет,
 * и это лучше пустоты: стадия остаётся читаемой, пока никто не дошёл
 * до её перевода.
 */
export function statusLabel(
  ...
  status: {
    name: string;
    /**
     * Обязателен намеренно. Пока поле было необязательным, половина экранов
     * собирала объект статуса руками и его не передавала — имя показывалось
     * верно на доске и неверно в списке объектов и в сводке. Обязательное
     * поле превращает такой промах в ошибку компиляции.
     */
