...
/**
 * Насыщенные градиенты для плиток, заливаемых цветом целиком.
 *
 * Отдельно от `PALETTE`: там пары «тёмный текст на светлой заливке», здесь
 * наоборот — белый текст на цвете. Одна пара для обоих случаев не годится,
 * потому что светлая заливка палитры под белым текстом нечитаема.
 *
 * Каждый градиент — два соседних оттенка одного тона, а не два разных цвета:
 * радуга в плитке выглядит нарядно ровно один раз, а потом мешает.
 */
const GRADIENTS = [
  'linear-gradient(135deg, #7c3aed 0%, #a855f7 100%)',
  'linear-gradient(135deg, #0c836f 0%, #14b8a6 100%)',
  'linear-gradient(135deg, #1f6feb 0%, #38bdf8 100%)',
  'linear-gradient(135deg, #c2255c 0%, #f43f5e 100%)',
  'linear-gradient(135deg, #b36200 0%, #f59e0b 100%)',
  'linear-gradient(135deg, #5b53c9 0%, #818cf8 100%)',
] as const;

export function gradientOf(seed: string): string {
  const index = hueOf(seed) % GRADIENTS.length;
  return GRADIENTS[index] ?? GRADIENTS[0];
}

/**
 * Градиент по НОМЕРУ, а не по имени.
 *
 * Для главных плиток сводки порядок задаётся руками: первым идёт фирменный
 * фиолетовый, потому что первое число — главное, и оно должно быть цвета
 * продукта. Отданный на откуп хешу, он выпадал куда придётся, и на экране
 * не оставалось ни одного фирменного пятна.
 */
