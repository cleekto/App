...
import { UnreadBadge, UnreadDot, UnreadProvider } from './unread';
...
export default async function AppLayout({ children }: { children: ReactNode }) {
  ...
  return (
    /*
     * Непрочитанное считается ОДИН раз на всю оболочку и раздаётся значкам:
     * их четыре — «чат», «сообщения», точка на бургере и точки на комнатах, —
     * и свой опрос у каждого означал бы четыре одинаковых запроса подряд.
     */
    <UnreadProvider>
        ...
        */}
          <header className="sticky top-0 z-10 flex items-center justify-between gap-3 border-b border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-3 md:hidden">
            ...
            <label
              htmlFor={NAV_TOGGLE_ID}
              aria-label={t('nav.menu')}
              className="relative flex size-9 shrink-0 items-center justify-center rounded-[var(--radius-control)] text-[var(--color-text-secondary)] hover:bg-[var(--color-surface-muted)] hover:text-[var(--color-text-primary)]"
            >
              <svg
                viewBox="0 0 24 24"
                fill="none"
              ...
              {/* На телефоне панель спрятана, и значки в ней не видны вовсе:
                новое сообщение приходило беззвучно, пока человек сам
                не открывал меню. */}
              <UnreadDot label={t('nav.unread')} />
            </label>
          </header>
