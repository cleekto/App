...
export interface WorkFeedItem {
  ...
  /** Сколько раз объявление смотрели на площадке. `null` — не выясняли. */
 ...
 * Порядок ленты.
 *
 * `new` — свежее сверху, по дате публикации площадки.
 *
 * `quiet` — сначала те, где ещё никого не было: по счётчику просмотров,
 * снизу вверх. Это единственный признак «заезженности», которым продавец
 * не управляет — цену и дату поднятия он двигает сам, а просмотры нет.
 * Объявление с тремя тысячами просмотров видел весь город, и собственнику
 * уже звонили полсотни раз.
 *
 * Оговорка, которую честнее сказать, чем спрятать: просмотры копятся
 * со временем, поэтому у свежего их мало просто потому, что оно свежее.
 * Для «кому ещё не звонили» это и нужно, но «самое непопулярное» этот
 * порядок не показывает — и не притворяется, что показывает.
 */
export type FeedOrder = 'new' | 'quiet';

export interface WorkFeedFilters {
  stream?: FeedStream | undefined;
  order?: FeedOrder | undefined;
...
export async function workFeed(
  ...
  const rows = await prisma.listingObservation.findMany({
    ...
    orderBy:
      (filters.order ?? 'new') === 'quiet'
        ? /*
           * СНАЧАЛА ТЕ, ГДЕ ЕЩЁ НИКОГО НЕ БЫЛО.
           *
           * Объявления без выясненного счётчика уходят вниз, а не наверх:
           * «не знаю» не должно притворяться нулём и занимать первые строки,
           * которые агент прочитает первыми.
           */
          [{ viewCount: { sort: 'asc', nulls: 'last' } }, { publishedAt: 'desc' }]
        : /*
           ...
           * объявления, созданные годы назад, — на сохранённой странице
           * третьим шло объявление 2023 года. Поэтому просмотр сайта нового
           * и не находит. `publishedAt` — настоящая дата публикации.
           *
           * Кто ещё не опознан по дате, уходит вниз, а не наверх.
           *
           ...
           */
          [{ publishedAt: { sort: 'desc', nulls: 'last' } }, { firstSeenAt: 'desc' }],
