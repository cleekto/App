...
export function scopeFilter(
  ...
  switch (scope) {
    ...
    case 'team': {
      ...
      // это честнее, чем показать всё. `field: null` Prisma отклоняет
      // как ошибку валидации, если колонка обязательная (`Property.teamId`
      // и не только) — `IN ()` даёт то же «ничего не найдено», не упираясь
      // в тип конкретной колонки. Найдено первым же ручным входом агентом
      // без команды: юнит-тест проверял форму фильтра, а не то, что Prisma
      // с ним делает.
      base[field] = ctx.teamId === null ? { in: [] } : ctx.teamId;
      return base;
    }
