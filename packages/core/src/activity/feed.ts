...
export async function listFollowUps(ctx: AuthContext, limit = 50): Promise<FollowUp[]> {
  ...
  /*
   * СВЯЗЬ С СОБСТВЕННИКОМ ЛИЧНАЯ (решение владельца 2026-09-06).
   * Договорился связаться — тебе и напоминают, а не всей команде.
   *
   * ЧЬЯ ЭТО СВЯЗЬ, БЕРЁТСЯ ИЗ `updatedByUserId` — колонки «владелец»
   * у состояния нет и быть не может: оно привязано к паре «объявление ×
   * команда» (`@@unique([observationId, teamId])`), то есть двух состояний
   * на одно объявление внутри команды не существует. Поэтому «моё» — это
   * «я последним отметил», и другого смысла у него в этой модели нет.
   *
   * СЛЕДСТВИЕ, О КОТОРОМ НАДО ЗНАТЬ: если по этому же объявлению отметится
   * коллега, напоминание перейдёт к нему. Разделить связи между двумя
   * агентами одной команды нельзя без нового поля и миграции — и это
   * не настройка прав, а изменение модели.
   *
   * Область складывается через `AND`: слияние объектов позволило бы
   * условию «моё» затереть границу компании.
   */
  const area: Prisma.ObservationStateWhereInput[] =
    scope === 'own'
      ? [{ companyId: ctx.companyId, updatedByUserId: ctx.userId }]
