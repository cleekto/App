...
export const ru: DeepPartial<Dictionary> = {
  ...
  property: {
    ...
    copy: 'Копировать',
    copied: 'Скопировано',
    listingFields: 'Значения для формы',
    listingFieldsHint:
      'В том порядке, в каком их спрашивает площадка, готовы к вставке — без валюты и разделителей разрядов.',
    publishNextTitle: 'Что делать дальше',
    publishNextForm: 'Форма объявления открылась в новой вкладке.',
    publishNextFormSelf:
      'Откройте на площадке форму нового объявления сами — эту kleekTo открыть не умеет.',
    publishNextFill: 'Расширение заполняет поля. Что не смогло — перечислит, это допишите сами.',
    publishNextManual:
      'Эта площадка заполняется руками. Скопируйте текст и значения с этой карточки.',
    publishNextPhotos:
      'Фотографии лежат в папке «Загрузки» — перетащите их в форму. Ещё не скачали? Кнопка «Скачать все фото» выше.',
    publishNextPublish:
      'Проверьте всё и нажмите «Опубликовать» сами. kleekTo форму не отправляет никогда.',
    publishNextGot: 'Понятно',
  ...
  extension: {
    ...
    fill: {
      ...
      waiting: 'Пройдите первые шаги: категория, тип, сделка. Заполню, как только появятся поля.',
